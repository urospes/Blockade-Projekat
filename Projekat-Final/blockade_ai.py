...
class BlockadeAI:
    ...
    def check_for_paths(self) -> bool:
        ...
        self.check_all_paths(tuple(self.game.board.player2.positions[0]), tuple(self.game.board.player2.positions[1]), tuple(self.game.board.startPositionsX[0]), tuple(self.game.board.startPositionsX[1]), self.dist_red1, self.dist_red2, False)
        ...
        if wallType == 'p':
            for i in range(0, 2):
                if (wallPos[0] - 1, wallPos[1]) in self.prev_paths[i] and (wallPos[0] + 1, wallPos[1]) in self.prev_paths[i]:
                    return True
                if (wallPos[0], wallPos[1]) in self.prev_paths[i] and ((wallPos[0] + 2, wallPos[1]) in self.prev_paths[i] or (wallPos[0] + 1, wallPos[1] + 1) in self.prev_paths[i] or ((wallPos[0] + 1, wallPos[1] - 1) in self.prev_paths[i] and ((wallPos[0] - 1, wallPos[1] - 1, 'z') in self.game.board.walls or (wallPos[0], wallPos[1] - 2, 'p') in self.game.board.walls) )):
                    return True
                if (wallPos[0] - 1, wallPos[1] + 1) in self.prev_paths[i] and (wallPos[0] + 1, wallPos[1] + 1) in self.prev_paths[i]:
                    return True
                if (wallPos[0], wallPos[1] + 1) in self.prev_paths[i] and ((wallPos[0] + 2, wallPos[1] + 1) in self.prev_paths[i] or (wallPos[0] + 1, wallPos[1]) in self.prev_paths[i] or ((wallPos[0] + 1, wallPos[1] + 2) in self.prev_paths[i] and ((wallPos[0] - 1, wallPos[1] + 1, 'z') in self.game.board.walls or (wallPos[0], wallPos[1] + 2, 'p') in self.game.board.walls))):
                    return True
        else:
            for i in range(0, 2):
                if (wallPos[0], wallPos[1] - 1) in self.prev_paths[i] and (wallPos[0], wallPos[1] + 1) in self.prev_paths[i]:
                    return True
                if (wallPos[0], wallPos[1]) in self.prev_paths[i] and ((wallPos[0], wallPos[1] + 2) in self.prev_paths[i] or (wallPos[0] + 1, wallPos[1] + 1) in self.prev_paths[i] or ((wallPos[0] - 1, wallPos[1] + 1) in self.prev_paths[i] and ((wallPos[0] - 1, wallPos[1] - 1, 'p') in self.game.board.walls or (wallPos[0]-2, wallPos[1], 'z') in self.game.board.walls))):
                    return True
                if (wallPos[0] + 1, wallPos[1] - 1) in self.prev_paths[i] and (wallPos[0] + 1, wallPos[1] + 1) in self.prev_paths[i]:
                    return True
                if (wallPos[0] + 1, wallPos[1]) in self.prev_paths[i] and ((wallPos[0] + 1, wallPos[1] + 2) in self.prev_paths[i] or (wallPos[0], wallPos[1] + 1) in self.prev_paths[i] or ((wallPos[0] + 2, wallPos[1] + 1) in self.prev_paths[i] and ((wallPos[0] - 1, wallPos[1] - 1, 'p') in self.game.board.walls or (wallPos[0]+2, wallPos[1], 'z') in self.game.board.walls))):
                    return True
        return False
    ...
    def minmax(self, state, depth, alpha, beta, max_move, prev_state):
        ...
        if max_move:
            ...
            for next_state in self.generateNextGameStates(state):
                ...
                next_val = self.minmax(next_state, depth - 1, alpha, beta, False, prev_state)[1]
                if max_val < next_val:
                    max_val = next_val
            ...
            for next_state in self.generateNextGameStates(state):
                prev_state[next_state] = state
                next_val = self.minmax(next_state, depth - 1, alpha, beta, True, prev_state)[1]
                if min_val > next_val:
                   min_val = next_val
                   best_move = next_state
