...
def DrawLabels(game_rect: pygame.Rect, size: tuple[int, int], offset: int):
    ...
    for i in range(0, size[0]):
        ...
        label_rect = pygame.rect.Rect(game_rect.left + const.WALL_W + offset, game_rect.top + (i + 1) * (const.SQUARE + const.WALL_W), const.SQUARE, const.SQUARE)
        screen.blit(label_surface, label_rect)

    #oznake kolona
    for i in range(0, size[1]):
        label_surface = label_font.render(str(labels[i]), False, "WHITE")
        label_rect = pygame.rect.Rect(game_rect.left + (i + 1) * (const.WALL_W + const.SQUARE) + offset, game_rect.top + const.WALL_W, const.SQUARE, const.SQUARE)
        screen.blit(label_surface, label_rect)
...
def DrawGameBoard(size: tuple[int, int], player_1: tuple[tuple[int, int]], player_2: tuple[tuple[int, int]], walls: list[tuple[int, int]], start_positions):
    ...
    board_rect = pygame.rect.Rect(game_rect.left + const.WALL_W + const.SQUARE + (game_rect.width - board_w) // 2, game_rect.top + const.WALL_W + const.SQUARE, board_w, board_h)
    ...
    p1_f1_rect = pygame.draw.circle(screen, const.RED_PLAYER, (c_x, c_y), const.SQUARE // 3)
    ...
    p1_f2_rect = pygame.draw.circle(screen, const.RED_PLAYER, (c_x, c_y), const.SQUARE // 3)
    ...
    p2_f1_rect = pygame.draw.circle(screen, const.YELLOW_PLAYER, (c_x, c_y), const.SQUARE // 3)
    ...
    p2_f2_rect = pygame.draw.circle(screen, const.YELLOW_PLAYER, (c_x, c_y), const.SQUARE // 3)
...
def PrintWinner(winner_red: bool):
    ...
    winner_text_surface = winner_font.render(winner_text, True, const.WIN_TEXT_COLOR)
    winner_text_rect = winner_text_surface.get_rect(center = (const.WIN_WIDTH // 2, const.WIN_HEIGHT // 2))
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOVE_FINISHED:
            ...
            wall_type = move_state.wall_pos[2] if len(move_state.wall_pos) != 0 else ''
            player_type = 'x' if move_state.red_to_move else 'o'
            move_done = game.nextMove(move_state.figure_num, wall_move_pos, wall_type, player_type, move_state.new_figure_pos)
            if game.isEnd():
                game_end = True
            else:
    ...
    if not game_end:
        #draw
        screen.fill("BLACK")
        DrawHeader((game.board.player1.greenWallNumber, game.board.player1.blueWallNumber), (game.board.player2.greenWallNumber, game.board.player2.blueWallNumber), move_state.red_to_move)
        board_info = DrawGameBoard((game.board.m, game.board.n), game.board.player1.positions, game.board.player2.positions, game.board.walls, (game.board.startPositionsX, game.board.startPositionsO))
        move_state.board_info = board_info
        DrawPossibleMoves(move_state.valid_moves, move_state.new_figure_pos, move_state.board_info["board_rect"])
    else:
